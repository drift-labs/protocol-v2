...
import { Connection, Commitment, BlockhashWithExpiryBlockHeight } from '@solana/web3.js';
...
describe('CachedBlockhashFetcher', () => {
  ...
  it('should retry on failure', async () => {
    connection.getLatestBlockhash
      ...
      .onThirdCall().resolves(mockBlockhash);
  ...
  it('should throw error after maximum retries', async () => {
    ...
    try {
      ...
      expect(error.message).to.equal('Failed to fetch blockhash after maximum retries');
    }
    expect(connection.getLatestBlockhash.calledThrice).to.be.true;
  });
