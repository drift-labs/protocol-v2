...
describe("TransactionConfirmationManager", () => {
  ...
  beforeEach(() => {
    ...
    manager = new TransactionConfirmationManager(mockConnection as unknown as Connection);
  });
  ...
  it("should resolve when transaction is confirmed", async () => {
    ...
    const result = await manager.confirmTransaction(fakeTxSig, "confirmed", 30000, 400);

    expect(result).to.deep.equal(fakeStatus);
    expect(mockConnection.getSignatureStatuses.calledWith([fakeTxSig], { searchTransactionHistory: false })).to.be.true;
  });
  ...
  it("should reject on timeout", async () => {
    ...
    const promise = manager.confirmTransaction(fakeTxSig, "confirmed", 5000, 1000);
  ...
  it("should check multiple transactions together", async () => {
    ...
    const promise1 = manager.confirmTransaction(fakeTxSig1, "confirmed", 30000, 400);
    const promise2 = manager.confirmTransaction(fakeTxSig2, "confirmed", 30000, 400);
    ...
    expect(mockConnection.getSignatureStatuses.calledWith([fakeTxSig1, fakeTxSig2], { searchTransactionHistory: false })).to.be.true;
  });
  ...
  it("should have overlapping request for transactions with 400ms and 1200ms intervals on the third 400ms interval", async function() {
    ...
    const promise1 = manager.confirmTransaction(fakeTxSig1, "confirmed", 5000, 400);
    const promise2 = manager.confirmTransaction(fakeTxSig2, "confirmed", 5000, 1200);
    ...
    assert(Math.abs(overlapTime - 1200) < 100, `Overlapping call should be around 1200ms, but was at ${overlapTime}ms`);

    // Verify the call pattern
    assert(
      callSignatures[0].includes(fakeTxSig1) && !callSignatures[0].includes(fakeTxSig2),
      "First call should only include 400ms interval transaction"
    );
    assert(
      callSignatures[1].includes(fakeTxSig1) && !callSignatures[1].includes(fakeTxSig2),
      "Second call should only include 400ms interval transaction"
    );
    assert(
      callSignatures[2].includes(fakeTxSig1) && callSignatures[2].includes(fakeTxSig2),
    ...
    assert.strictEqual(callTimes.length, 3, "Should not have made any more calls after all transactions are confirmed");

    // Verify that only the third call returns non-null results
    callCount = 0;
    const results = await Promise.all(callSignatures.map(sigs => 
      mockConnection.getSignatureStatuses!(sigs)
    ));
    
    assert(results[0].value.every(v => v === null), "First call should return null results");
    assert(results[1].value.every(v => v === null), "Second call should return null results");
    assert(results[2].value.every(v => v !== null), "Third call should return non-null results");
  });
});