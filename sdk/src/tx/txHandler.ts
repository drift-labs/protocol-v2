...
export class TxHandler {
    ...
    private async signTx(
        ...
        // @ts-ignore
        console.log(`⭐️:: signing txs with last block height : ${tx.DRIFT_SIGNED_WITH_BLOCK_AND_EXP.lastValidBlockHeight}`);
	...
	public async buildTransaction(
        ...
        if (process.env.DEV_TRY_FORCE_TX_TIMEOUTS) {
            allIx.push(
    ...
    public async getSignedTransactionMap(
        ...
        signedTxs.forEach((signedTx, index) => {
            // @ts-ignore
            signedTx.DRIFT_SIGNED_WITH_BLOCK_AND_EXP = txsToSign[index]?.DRIFT_SIGNED_WITH_BLOCK_AND_EXP;
        });

        // @ts-ignore
        console.log(`⭐️:: signing txs with last block height : ${signedTxs[0].DRIFT_SIGNED_WITH_BLOCK_AND_EXP.lastValidBlockHeight}`);
    ...
    public async buildAndSignTransactionMap(
        ...
        const preppedTransactions = await (props.txVersion === 'legacy' ? this.getPreparedAndSignedLegacyTransactionMap(
