...
export class AdminClient extends DriftClient {
	...
	public async initializeOraclePriceCache(
		...
		const ix = await this.getInitializeOraclePriceCacheIx(cacheId, numOracles);
		const tx = await this.buildTransaction(ix, txParams);
		const { txSig } = await this.sendTransaction(tx, [], this.opts);
		return txSig;
	}
	...
	public async getSetOracleCacheEntriesIx(
		...
		return await this.program.instruction.setOracleCacheEntries(anchorEntries, {
			accounts: {
				admin: this.useHotWalletAdmin
					? this.wallet.publicKey
					: this.getStateAccount().admin,
	...
	public async updateOracleCacheConfig(
		...
		const ix = await this.getUpdateOracleCacheConfigIx(cacheId, maxAgeSlots);
		const tx = await this.buildTransaction(ix, txParams);
		const { txSig } = await this.sendTransaction(tx, [], this.opts);
		return txSig;
	}
	...
	public async getUpdateOracleCacheConfigIx(
		...
		return await this.program.instruction.updateOracleCacheConfig(maxAgeSlots, {
			accounts: {
				admin: this.useHotWalletAdmin
					? this.wallet.publicKey
					: this.getStateAccount().admin,
