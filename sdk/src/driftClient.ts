...
export class DriftClient {
	...
	public getOracleDataForPerpMarket(marketIndex: number): OraclePriceData {
		...
		if (!oraclePriceDataAndSlot) {
			throw new Error(
				`Oracle price data and slot not found for market index ${marketIndex}`
			);
		}
	...
	public getMMOracleDataForPerpMarket(marketIndex: number): MMOraclePriceData {
		...
		if (!oracleData) {
			throw new Error(`Oracle data not found for market index ${marketIndex}`);
		}
	...
	public getMarketFees(
		...
		if (marketIndex !== undefined) {
			...
			if (isVariant(marketType, 'perp')) {
				...
				feeAdjustment =
					(marketAccount as { feeAdjustment?: number })?.feeAdjustment ?? 0;
			}
