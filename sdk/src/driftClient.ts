...
import { WebSocketDriftAccountSubscriber } from './accounts/webSocketDriftAccountSubscriber';
import { RetryTxSender } from './tx/retryTxSender';
import { DriftUser } from './driftUser';
import { DriftUserAccountSubscriptionConfig } from './driftUserConfig';
import { getMarketsAndOraclesForSubscription } from './config';
import { WRAPPED_SOL_MINT } from './constants/spotMarkets';
import { DriftUserStats } from './driftUserStats';
...
export class DriftClient {
	...
	opts?: ConfirmOptions;
	users = new Map<number, DriftUser>();
	userStats?: DriftUserStats;
	activeUserId: number;
	userAccountSubscriptionConfig: DriftUserAccountSubscriptionConfig;
	...
	public constructor(config: DriftClientConfig) {
		...
		if (config.userStats) {
			this.userStats = new DriftUserStats({
				driftClient: this,
		...
		if (config.accountSubscription?.type === 'polling') {
			...
			this.accountSubscriber = new WebSocketDriftAccountSubscriber(
				this.program,
	...
	createUsers(
		userIds: number[],
		accountSubscriptionConfig: DriftUserAccountSubscriptionConfig
	): void {
	...
	public async getStatePublicKey(): Promise<PublicKey> {
		...
		this.statePublicKey = await getStateAccountPublicKey(
			this.program.programId
		);
		return this.statePublicKey;
	}
	...
	/**
	 * Update the wallet to use for drift client transactions and linked user account
	 * @param newWallet
	 * @param userIds
	 * @param activeUserId
	 */
	...
	public getUser(userId?: number): DriftUser {
	...
	public getUsers(): DriftUser[] {
		return [...this.users.values()];
	}

	public getUserStats(): DriftUserStats {
