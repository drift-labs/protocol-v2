import { ClearingHouse, AMM_MANTISSA, BASE_ASSET_PRECISION } from './clearingHouse';
		...
		for (const marketIndex in marketsAccount.markets) {
			...
			for(const position in positions){
				if(positions[position].marketIndex.eq(marketIndexBN)){
					...
					netExposure = stripMantissa(arbPos.baseAssetAmount, BASE_ASSET_PRECISION);
					positionValue = this.userAccount.getPositionValue(positionIdx);
				}
				positionIdx+=1;
			}

			const isPositionValueLimit = positionValue.gt(this.userAccount.getTotalCollateral()); // no more than 1x leverage per market
			...
			this.alphas[marketIndexBN.toNumber()] = .99*this.alphas[marketIndexBN.toNumber()] + .01*(oraclePrice-oraclePrevPrice)/oracleConf;
			...
			const estFundingPayment = netExposure * stripMantissa((markTwapWithMantissa
				.sub(oracleTwapWithMantissa))) / 24;
			...
			if(closeToFundingUpdate){
				...
				if((estFundingPayment > 0 && netExposure > 0)
			...
			if(((netExposure > 0 && markPrice < oraclePrice) || 
				...
				if((currentSpreadPct < .0075 || currentSpread < oracleTwac || currentSpread < oracleNoise) && !shouldReducePosition){
					console.log('spread too small to arb in Market:', marketIndexBN.toNumber());
					continue;
				}
			}
            ...
            if((markPrice >= oracleTwap && markPrice < oraclePrice && netExposure > 0)
                || (markPrice <= oracleTwap && markPrice > oraclePrice && netExposure < 0)){
				arbPctMod = new BN(Math.min(randomDraw/2, 100));
            } else if((markPrice > oraclePrice && netExposure>0)
			...
			if(arbPctMod.gt(ZERO)){
				...
				const reductionSize = (positionValue.div(new BN(Math.sqrt(Math.max(2,nextFundingTime)))));
				amount = reductionSize;
				direction = netExposure > 0 ? PositionDirection.SHORT : PositionDirection.LONG;
			}
			...
			if (direction == PositionDirection.LONG) {
				...
				limitPrice = new BN(limitPrice.toNumber() * .9999);
			}
