...
export class TitanClient {
	...
	constructor({ connection, authToken, url }: { connection: Connection; authToken: string; url?: string }) {
	...
	public async getQuote({
		...
		const params = new URLSearchParams({
			...
			...(swapMode && { swapMode: swapMode === 'ExactOut' ? SwapMode.ExactOut : SwapMode.ExactIn }),
		...
		return {
			...
			otherAmountThreshold: Math.floor(route.outAmount * (1 - slippageBps / 10000)).toString(),
			...
			priceImpactPct: '0',
			routePlan: route.steps?.map((step: any) => ({
				swapInfo: {
	...
	public async getSwap({
		...
		onlyDirectRoutes?: boolean;
		excludeDexes?: string[];
	}): Promise<{transactionMessage: TransactionMessage, lookupTables: AddressLookupTableAccount[]}> {
		const params = new URLSearchParams({
			inputMint: inputMint.toString(),
			outputMint: outputMint.toString(),
			amount: amount.toString(),
    ...
    private async getTransactionMessageAndLookupTables(route: SwapRoute, userPublicKey: PublicKey): Promise<{transactionMessage: TransactionMessage, lookupTables: AddressLookupTableAccount[]}> {
        const solanaInstructions: TransactionInstruction[] = route.instructions.map(instruction => ({
        ...
        if (route.addressLookupTables && route.addressLookupTables.length > 0) {
            for (const altPubkey of route.addressLookupTables) {
                try {
                    const altAccount = await this.connection.getAddressLookupTable(new PublicKey(altPubkey));
                    if (altAccount.value) {
