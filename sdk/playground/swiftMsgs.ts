...
const generate_payload = async (driftClient: DriftClient, delegate: boolean, tpsl: boolean, withBuilder: boolean) => {
    ...
    const orderParamsMessage = {
        ...
        builderIdx: withBuilder ? 1 : null,
        builderFeeBps: withBuilder ? 8 : null,
    };
...
const main = async () => {
    ...
    // await generate_payload(driftClient, false, true, true);
    await generate_payload(driftClient, true, true, true);
    await generate_payload(driftClient, true, false, true);
};
