...
export const managerApplyProfitShare = async (
	program: Command,
	...
	if (cmdOpts.dumpTransactionMessage) {
		const tx = await velocityVault.getApplyProfitShareIx(
			vaultAddress,
			vaultDepositorAddress
		);
		console.log(dumpTransactionMessage(velocityClient.wallet.publicKey, [tx]));
	} else {
		const tx = await velocityVault.applyProfitShare(
			vaultAddress,
			vaultDepositorAddress
		);
		console.log(
			`Applied profit share: https://solana.fm/tx/${tx}${
