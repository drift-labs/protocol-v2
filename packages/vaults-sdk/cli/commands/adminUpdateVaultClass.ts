...
export async function adminUpdateVaultClass(
	program: Command,
	...
	try {
		if (dumpTx) {
			const ix = await velocityVault.getAdminUpdateVaultClassIx(
				vault,
				newVaultClass
			);
		...
		const txSig = await velocityVault.adminUpdateVaultClass(
			vault,
			newVaultClass
		);
