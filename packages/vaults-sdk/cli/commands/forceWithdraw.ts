...
export const forceWithdraw = async (
	program: Command,
	...
	try {
		...
		console.error(
			'Failed to parse vaultDepositorAddress trying vaultDepositorAuthority'
		);
	}

	let vaultDepositorAuthority: PublicKey | undefined;
	try {
		vaultDepositorAuthority = new PublicKey(
			cmdOpts.vaultDepositorAuthority as string
		);
	} catch (err) {
	...
	if (vaultDepositorAuthority && !vaultDepositorAddress) {
		try {
			...
			throw new Error(
				'Must provide --vault-address if only --vault-depositor-authority is provided'
			);
		}
	...
	const tx = await velocityVault.forceWithdraw(vaultDepositorAddress, {
		lookupTables: await velocityClient.fetchAllLookupTableAccounts(),
	});
