...
export const viewVaultDepositor = async (
	program: Command,
	cmdOpts: OptionValues
) => {
	let vaultDepositorAddress: PublicKey;

	const { velocityVault } = await getCommandContext(program, false);
	...
	try {
		if (cmdOpts.vaultDepositorAddress !== undefined) {
			...
			cmdOpts.authority !== undefined &&
			cmdOpts.vaultAddress !== undefined
		) {
			...
			console.error(
				'Must supply --vault-depositor-address or --authority and --vault-address'
			);
			process.exit(1);
		}
	} catch (err) {
	...
	const vaultDepositor = await velocityVault.getVaultDepositor(
