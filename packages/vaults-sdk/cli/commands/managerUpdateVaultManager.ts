...
export const managerUpdateVaultManager = async (
	program: Command,
	...
	while (!done) {
		try {
			if (cmdOpts.dumpTransactionMessage) {
				const tx = await velocityVault.getManagerUpdateVaultManagerIx(
					vaultAddress,
					manager
				);
				console.log(
				...
				const tx = await velocityVault.managerUpdateVaultManager(
					vaultAddress,
					manager
				);
				console.log(
					`Updated vault manager: https://solana.fm/tx/${tx}${
