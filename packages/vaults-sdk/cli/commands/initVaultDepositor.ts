...
export const initVaultDepositor = async (
	...
	const { velocityVault } = await getCommandContext(program, true);
	...
	const tx = await velocityVault.initializeVaultDepositor(
		vaultAddress,
		depositAuthority,
		velocityVault.velocityClient.wallet.publicKey
	);
