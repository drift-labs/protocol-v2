...
export const managerUpdateVaultDelegate = async (
	program: Command,
	...
	if (cmdOpts.dumpTransactionMessage) {
		...
		const tx = await velocityVault.getUpdateDelegateIx(
			vaultAddress,
			delegate,
			vaultAccount.user,
		...
		console.log(
			`Updated vault delegate to ${delegate.toBase58()}: https://solana.fm/tx/${tx}${
