	...
	for (const [i, order] of userAccount.orders.entries()) {
		...
		console.log(
			`  existingPositionDirection: ${
				Object.keys(order.existingPositionDirection)[0]
			}`
		);
		...
		console.log(
			`  oraclePriceOffset: ${convertToNumber(
		...
		console.log(
			`  triggerCondition: ${Object.keys(order.triggerCondition)[0]}`
		);
		...
		console.log(
			`  fee: ${convertToNumber(order.fee, BASE_PRECISION).toString()}`
		);
