...
import { AdminClient, DriftUser, PEG_PRECISION } from '../sdk/src';
...
describe('User Account', () => {
	...
	it('After Position Taken', async () => {
		...
		const oraclePrice = driftClient.getOracleDataForMarket(
			market.marketIndex
		).price;
		...
		const oraclePrice2 = driftClient.getOracleDataForMarket(
			market.marketIndex
		).price;
