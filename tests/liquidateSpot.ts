...
describe('liquidate spot', () => {
	...
	it('liquidate', async () => {
		...
		assert(!driftClient.getUserAccount().beingLiquidated); // out of liq territory
		...
		assert(driftClient.getUserAccount().spotPositions[0].balance.gt(ZERO));
		// assert(
		// 	driftClient.getUserAccount().spotPositions[1].balance.gt(new BN(2))
		// );
		// assert(
		// 	isVariant(
		// 		driftClient.getUserAccount().spotPositions[0].balanceType,
		...
		// );
		console.log(
			driftClient.getUserAccount().spotPositions[0].balance.toString()
		);
