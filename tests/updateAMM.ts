...
async function feePoolInjection(fees, marketIndex, driftClient) {
	...
	await driftClient.updateCurveUpdateIntensity(marketIndex, 0);
	...
	while (market0.amm.totalFeeMinusDistributions.lt(fees)) {
		const reservePrice = calculateReservePrice(
			market0,
			driftClient.getOracleDataForMarket(marketIndex)
		);
	...
	await driftClient.updateCurveUpdateIntensity(marketIndex, 100);
}
...
describe('update amm', () => {
	...
	it('update AMM (imbalanced, oracle > peg, sufficient fees)', async () => {
		...
		let txSig;
		try {
			txSig = await driftClient.placeAndTake(orderParams);
		} catch (e) {
			console.error(e);
		}
	...
	it('update AMMs (unbalanced, oracle > peg, cost > 0 and insufficient fees)', async () => {
		...
		const txSig3 = await driftClient.placeAndTake(orderParams);
