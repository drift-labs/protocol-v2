...
describe('referrer', () => {
	...
	before(async () => {
		...
		await referrerDriftClient.updatePerpAuctionDuration(0, 10);

		const periodicity = new BN(60 * 60); // 1 HOUR

		await referrerDriftClient.initializeMarket(
			solOracle,
	...
	it('fill order', async () => {
		const txSig = await refereeDriftClient.placeAndTake(
			getMarketOrderParams({
		...
		await refereeDriftClient.placeAndTake(
			getMarketOrderParams({
				baseAssetAmount: BASE_PRECISION,
