...
import { BASE_PRECISION, BN, MarketStatus, OracleSource, TestClient } from "../sdk/src";
import { TestBulkAccountLoader } from "../sdk/src/accounts/testBulkAccountLoader";
import { BankrunContextWrapper } from "../sdk/src/bankrunConnection";
import { startAnchor } from 'solana-bankrun';
import { AccountInfo, LAMPORTS_PER_SOL, PublicKey } from '@solana/web3.js';
import { DEFAULT_RECEIVER_PROGRAM_ID } from '@pythnetwork/pyth-solana-receiver';
import { PYTH_ORACLE_ONE_DATA, PYTH_ORACLE_TWO_DATA } from './pythPullOracleData';
...
describe('pyth pull oracles', () => {
    ...
    before(async () => {
        ...
        driftClient = new TestClient({                  // call toConnection to avoid annoying type error
    ...
    it('parses prices', async () => {
        ...
        assert(priceDataOne.price.eq(new BN(148563151)), `priceDataOne.price: ${priceDataOne.price.toString()}`);
        assert(priceDataOne.slot.eq(new BN(271269395)), `priceDataOne.slot: ${priceDataOne.slot.toString()}`);

        assert(priceDataTwo.price.eq(new BN(134677319)), `priceDataTwo.price: ${priceDataTwo.price.toString()}`);
        assert(priceDataTwo.slot.eq(new BN(272607101)), `priceDataTwo.slot: ${priceDataTwo.slot.toString()}`);
    });
});