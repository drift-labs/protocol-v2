...
describe('minimum trade size', () => {
    ...
    it('short round', async () => {
        ...
        let user: UserAccount = await primaryClearingHouse.program.account.user.fetch(
            userAccountPublicKey
        );
        assert(user.collateral.eq(usdcAmount));

        let userPositions: UserPositionsAccount = await primaryClearingHouse.program.account.userPositions.fetch(
            user.positions
        );

        // make price slightly worse so we need to round trade amount
        const newBaseAssetReserve = ammInitialBaseAssetReserve.mul(new BN(9999999)).div(new BN(10000000));
        ...
        userPositions = await primaryClearingHouse.program.account.userPositions.fetch(
            user.positions
        );
    ...
    it('short dont round', async () => {
        ...
        const newBaseAssetReserve = ammInitialBaseAssetReserve.mul(new BN(1)).div(new BN(2));
        const newQuoteAssetReserve = invariant.div(newBaseAssetReserve);
        await primaryClearingHouse.moveAmmPrice(
            newBaseAssetReserve,
            newQuoteAssetReserve,
    ...
    it('long round', async () => {
        ...
        // make price slightly worse so we need to round trade amount
        const newQuoteAssetReserve = ammInitialBaseAssetReserve.mul(new BN(9999999)).div(new BN(10000000));
    ...
    it('short dont round', async () => {
        ...
        const newQuoteAssetReserve = ammInitialBaseAssetReserve.mul(new BN(4)).div(new BN(3));
        const newBaseAssetReserve = invariant.div(newQuoteAssetReserve);
        await primaryClearingHouse.moveAmmPrice(
            newBaseAssetReserve,
            newQuoteAssetReserve,
