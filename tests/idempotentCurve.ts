...
describe('idempotent curve', () => {
    ...
    const shrinkProfitableLong = async (chunks: number) => {
        ...
        const userUSDCAccount = await mockUserUSDCAccount(usdcMint, usdcAmount, provider, userKeypair.publicKey);
        ...
        const baseAssetValue = await clearingHouse.calculateBaseAssetValue(userPositionsAccount.positions[0]).div(AMM_MANTISSA);
