...
describe('repeg and spread amm', () => {
	...
	it('5 users, 15 trades, single market, user net lose, check invariants', async () => {
		// create <NUM_USERS> users with 10k that collectively do <NUM_EVENTS> actions
		const clearingHouseOld = clearingHouse;
		...
		console.log(
			'sum all amm/user pnl accounting:',
			sinceStartTFMD,
			'+',
			allUserUnsettledPnl,
			'==',
			pnlPoolBalance + feePoolBalance
		);
		...
		// todo: doesnt add up perfectly (~$2 off), adjust peg/k not precise?
		// make sure its negative (accounting for amm underestimates balances for it)
		assert(
			allUserUnsettledPnl +
				(sinceStartTFMD - (pnlPoolBalance + feePoolBalance)) <
				0
