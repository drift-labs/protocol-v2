...
async function iterClosePosition(
	...
	while (!userPosition.baseAssetAmount.eq(ZERO)) {
		...
		const txClose = await driftClient.placeAndTake(closeOrderParams);
		console.log(
			'tx logs',
			(
...
describe('repeg and spread amm', () => {
	...
	before(async () => {
		...
		await driftClient.updateMarketBaseSpread(0, 250);
		...
		// 		201
		// 	);
		// 	await driftClient.updateMarketBaseSpread(new BN(i), 2000);
		// 	await driftClient.updateCurveUpdateIntensity(new BN(i), 100);
		// }
	...
	it('BTC market massive spread', async () => {
		...
		// await driftClient.placeAndFillOrder(orderParams);
		// await driftClient.closePosition(new BN(0));
		const txSig0 = await driftClient.placeAndTake(orderParams);
		...
		console.log(
			...
			driftUser.getSpotMarketAssetValue().toString()
		);
		...
		console.log(
			driftClient.getUserAccount().perpPositions[0].quoteEntryAmount.toString()
		);
		// assert(
		// 	driftClient.getUserAccount().perpPositions[0].quoteAssetAmount.toString() ==
		// 		'157582183'
		...
		// assert(driftUser.getSpotMarketAssetValue().eq(new BN('10000000000'))); // remainder is of debt is for fees for revenue pool
	...
	it('5 users, 15 trades, single market, user net win, check invariants', async () => {
		...
		while (count < 15) {
			...
			await driftClients[count % 5].placeAndTake(orderParams);
			count += 1;
		}
		...
		const userCollateral = convertToNumber(
			driftUser.getSpotMarketAssetValue(),
			QUOTE_PRECISION
		);
		...
		for (let i = 0; i < driftClients.length; i++) {
			...
			const driftUserI = _userAccountInfos[i];
			const userCollateral = convertToNumber(
				driftUserI.getSpotMarketAssetValue(),
				QUOTE_PRECISION
			);

			const unsettledPnl = driftUserI
				.getUserAccount()
				.perpPositions.reduce((unsettledPnl, position) => {
					return unsettledPnl.add(
						position.quoteAssetAmount.add(position.quoteEntryAmount)
