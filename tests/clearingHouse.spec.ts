...
describe('clearing_house', () => {
	...
	it('SHORT order partial fill limit price ', async () => {
		...
		const newUSDCNotionalAmount = usdcAmount.div(new BN(2)).mul(new BN(5));
		assert(newUSDCNotionalAmount.gt(new BN(0)));
		const marketIndex = new BN(0);
		const estTradePrice =
			clearingHouse.calculateBaseAssetPriceAfterSwapWithMantissa(
				marketIndex,
		...
		// trying to sell at price too high
		const limitPriceMidPartial = clearingHouse
			.calculateBaseAssetPriceWithMantissa(marketIndex)
			.add(estTradePrice)
			.div(new BN(2));
		console.log(
			'*SHORT with partial fill*',
			'new price if no limit:',
			estTradePrice.toNumber(),
			'limit price for partial order:',
			limitPriceMidPartial.toNumber()
		);
		...
		console.log(
			'partial fill:',
			userPositionsAccount.positions[0].quoteAssetNotionalAmount.toNumber(),
			newUSDCNotionalAmount.toNumber()
		);
		assert.ok(
			userPositionsAccount.positions[0].quoteAssetNotionalAmount.lt(
				new BN(newUSDCNotionalAmount)
			)
		);
	});
});
