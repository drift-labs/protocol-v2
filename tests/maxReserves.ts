...
import { AMM_MANTISSA, ClearingHouse, PositionDirection, USDC_PRECISION } from '../sdk/src';
...
describe('max reserves', () => {
    ...
    const usdcAmount = (new BN(USDC_PRECISION)).mul(mantissaSqrtScale).mul(new BN(1));
    ...
    it('liquidate', async () => {
        ...
        for (let i  = 0; i < maxPositions; i++) {
            ...
            await clearingHouse.moveAmmToPrice(
                new BN(i),
                new BN(.1 * AMM_MANTISSA.toNumber())
            );
        }
