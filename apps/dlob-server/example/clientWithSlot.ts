...
const dlobServerURL = 'https://dlob.velocity.exchange/orders/idlWithSlot';
const rpcEndpoint = 'https://api.mainnet-beta.solana.com';
const velocityEnv = 'mainnet-beta';

// const dlobServerURL = "https://dlob.master.velocity.exchange/orders/idlWithSlot";
...
if (useWebsocket) {
	...
	accountSubscription = {
		type: 'polling',
		accountLoader: new BulkAccountLoader(connection, stateCommitment, 10000),
	};
	logProviderConfig = {
		type: 'polling',
		frequency: 10000,
		...
		if (countAsks < levelsToPrint) {
			console.log(
				` [${countAsks - 1}] ${
					...
					ask.getPrice(oracle, currSlot),
					PRICE_PRECISION
				)} (oraclePriceOffset: ${
					ask.order?.oraclePriceOffset || 0 * PRICE_PRECISION
				})`
			);
		}
		countAsks--;
		...
		if (countBids < levelsToPrint) {
			console.log(
				` [${countBids}] ${
					...
					bid.getPrice(oracle, currSlot),
					PRICE_PRECISION
				)} (oraclePriceOffset: ${
					bid.order?.oraclePriceOffset || 0 * PRICE_PRECISION
				})`
			);
		}
		countBids++;
...
function doDlobDemo() {
	...
	fetch(dlobServerURL).then(async (r) => {
