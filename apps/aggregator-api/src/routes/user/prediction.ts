import { PredictionRecord, RecordTypes } from '@backend/common';
...
const predictionRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const { records, meta } = await getPredictionRecords({ id: accountId, page: nextPage });
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const nextPage = request.getPaginationToken();
			const { records, meta } = await getPredictionRecordsBySymbol({
				id: accountId,
				symbol,
				page: nextPage,
			});
	...
	fastify.get<{
		...
		'/:year/:month',
		{
			...
			schema: {
				description:
					'<p>Retrieve prediction records for a specific account for a given year and month.</p>',
				tags: ['User'],
				params: {
					type: 'object',
					properties: {
