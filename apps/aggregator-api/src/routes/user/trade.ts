import { RecordTypes, TradeRecord } from '@backend/common';
...
const tradeRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
	const { getTradeRecords, getTradeRecordsBetweenTimestamps, getTradeRecordsBySymbol } =
		TradeRepository();
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const { records, meta } = await getTradeRecords({ id: accountId, page: nextPage });
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const nextPage = request.getPaginationToken();
			const { records, meta } = await getTradeRecordsBySymbol({
				id: accountId,
				symbol,
				page: nextPage,
			});
