import { LiquidationRecord, RecordTypes } from '@backend/common';
import { LiquidationRepository } from '@backend/dynamodb';
import { FastifyPluginAsync } from 'fastify';
import { archivedMetadataSchema, liquidationSchema, metadataSchema } from '../../schemas';
...
const liquidationRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
	const { getLiquidationRecords, getLiquidationRecordsBetweenTimestamps } =
	...
	fastify.get<{
		...
		'/:year/:month',
		{
			...
			schema: {
				description:
					'<p>Retrieve liquidation records for a specific account for a given year and month.</p>',
				tags: ['User'],
				params: {
					type: 'object',
					properties: {
		...
		async function (request, reply) {
			...
			const result = await fetchArchiveData<LiquidationRecord>({
				id: accountId,
				year,
				month,
				page,
				recordType: RecordTypes.LiquidationRecord,
				getLatestRecords: getLiquidationRecordsBetweenTimestamps,
			});
