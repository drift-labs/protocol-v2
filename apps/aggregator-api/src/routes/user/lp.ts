import { LPRecord, RecordTypes } from '@backend/common';
import { LPRepository } from '@backend/dynamodb';
import { FastifyPluginAsync } from 'fastify';
import { archivedMetadataSchema, lpSchema, metadataSchema } from '../../schemas';
import { fetchArchiveData } from '../../utils/fetch-archive-data';

const lpRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
	// LP Routes
	const { getLPRecords, getLPRecordsBetweenTimestamps } = LPRepository();
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const { records, meta } = await getLPRecords({ id: accountId, page: nextPage });
	...
	fastify.get<{
		...
		'/:year/:month',
		{
			...
			schema: {
				description:
					'<p>Retrieve LP records for a specific account for a given year and month.</p>',
				tags: ['User'],
				params: {
					type: 'object',
					properties: {
		...
		async function (request, reply) {
			...
			const result = await fetchArchiveData<LPRecord>({
				id: accountId,
				year,
				month,
				page,
				recordType: RecordTypes.LPRecord,
				getLatestRecords: getLPRecordsBetweenTimestamps,
			});
