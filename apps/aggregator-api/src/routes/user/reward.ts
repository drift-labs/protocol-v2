import { RecordTypes, RewardRecord } from '@backend/common';
...
const rewardRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
	...
	fastify.get<{
		...
		async function (request, reply) {
			...
			const { records, meta } = await getRewardRecords({ id: accountId, page: nextPage });
	...
	fastify.get<{
		...
		'/:year/:month',
		{
			...
			schema: {
				description:
					'<p>Retrieve reward records for a specific account for a given year and month.</p>',
				tags: ['User'],
				params: {
					type: 'object',
					properties: {
