...
pub struct Wallet {
    ...
    signer: Arc<Keypair>,
    /// The velocity 'authority' account
    /// user (sub)accounts are derived from this
    authority: Pubkey,
    /// The velocity 'stats' account
    stats: Pubkey,
    /// Wallet operation mode
    mode: Mode,
}
...
impl Wallet {
    ...
    /// * `authority` - velocity account to sign for (the delegator)
    ...
    /// Return the velocity user stats address
    ...
    /// Calculate the velocity user address given a `sub_account_id`
