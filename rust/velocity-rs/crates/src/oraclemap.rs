...
/// Dynamic map of Velocity market oracle data
///
...
impl OracleMap {
    ...
    /// * `all_oracles` - Exhaustive list of all Velocity oracle pubkeys and source by market
    ///
    pub fn new(
...
mod tests {
    ...
    const SOL_PERP_ORACLE: Pubkey =
        solana_pubkey::pubkey!("2k3UHX6ehRFzx5fTVvbL6FwXhMjkucjJDL9MuVKLo8TV");
    ...
    async fn oraclemap_sync() {
        let all_oracles = vec![
            (
                MarketId::spot(0),
                solana_pubkey::pubkey!("Dai8hT1YRBBm5rBSJUSKcdR11psM55LVAkshbypfC4k4"),
    ...
    async fn oraclemap_subscribes() {
        ...
        assert!(map.is_subscribed(&MarketId::spot(0)));
