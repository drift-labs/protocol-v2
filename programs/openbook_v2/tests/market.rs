...
pub async fn place_order_and_execute(mut banks_client:
                                     &mut BanksClient,
                                     keypair:
                                     &Keypair,
                                     args: PlaceOrder,
    ...
    let place_order_ix = Instruction::new_with_bytes(
        ...
        vec![
            ...
            AccountMeta::new(spl_associated_token_account::get_associated_token_address(&keypair.pubkey(), mint).key(), false), // ata
            ...
            AccountMeta::new(if let Side::Bid = args.side {market_keys.market_quote_vault} else { market_keys.market_base_vault }, false), // if bid quote else base
...
pub async fn create_bids_asks_event_heap(mut banks_client: &mut BanksClient, keypair: &Keypair) -> (Pubkey, Pubkey, Pubkey){
...
pub async fn create_default_market(banks_client: &mut BanksClient,
                           keypair: &Keypair,
                           base_mint: &Pubkey,
                           quote_mint: &Pubkey,
                           bids: &Pubkey,
    ...
    banks_client.process_transaction(tx).await?;
    return Ok((market.pubkey(), event_authority, market_authority, market_base_vault, market_quote_vault))
}