...
pub fn get_sb_on_demand_price(
    ...
    let latest_oracle_submissions: Vec<switchboard_on_demand::OracleSubmission> =
        pull_feed_account_info.latest_submissions();
    let average_price = latest_oracle_submissions
        .iter()
        .map(|submission| submission.value())
        .sum::<i128>()
        / latest_oracle_submissions.len() as i128;
    ...
    let delay = clock_slot
        .cast::<i64>()?
        .safe_sub(latest_oracle_submissions[0].landed_at.cast()?)?;
