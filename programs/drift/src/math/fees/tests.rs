mod calculate_fee_for_taker_and_maker {
    ...
    #[test]
    fn high_leverage_mode_whitelisted_with_stake() {
        let quote_asset_amount = 100 * QUOTE_PRECISION_U64;
        ...
        assert_eq!(taker_fee, 60000);
        assert_eq!(maker_rebate, 30000);
        assert_eq!(fee_to_market, 24000);
        assert_eq!(filler_reward, 6000);
        assert_eq!(referrer_reward, 0);
        assert_eq!(referee_discount, 0);
    }
}
...
mod calcuate_fee_tiers {
    ...
    fn test_calc_taker_tiers() {
        ...
        let res: FeeTier = determine_user_fee_tier(
            &taker_stats,
            &fee_structure,
            &MarketType::Perp,
            false,
        ...
        let res: FeeTier =
            determine_user_fee_tier(&taker_stats, &fee_structure, &MarketType::Perp, true, false)
                .unwrap();
        ...
        let res: FeeTier =
            determine_user_fee_tier(&taker_stats, &fee_structure, &MarketType::Perp, true, true)
                .unwrap();

        assert_eq!(res.fee_numerator, 21);
        assert_eq!(res.fee_denominator, 100000);
