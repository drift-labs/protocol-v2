...
/// When there is a period with asymmetric funding, the amm will pay/receive funding from/to it's collected fees.
pub fn calculate_funding_rate_long_short(
    ...
    // If the net market position owes funding payment, the amm receives payment
    ...
    // If the uncapped_funding_pnl is positive, the amm receives money.
    ...
    // amm is paying part of funding imbalance
    if capped_funding_pnl != 0 {
        let total_fee_minus_distributions_lower_bound =
            cast_to_i128(get_total_fee_lower_bound(market)?)?;

        // makes sure the amm doesn't pay more than the share of fees allocated to `distributions`
...
fn calculate_capped_funding_rate(
    market: &PerpMarket,
    uncapped_funding_pnl: i128, // if negative, users would net recieve from amm
    funding_rate: i128,
) -> DriftResult<(i128, i128)> {
    // The funding_rate_pnl_limit is the amount of fees the amm can use before it hits it's lower bound
    ...
    let capped_funding_rate = if uncapped_funding_pnl < funding_rate_pnl_limit {
        ...
        // this makes it so that the capped rate includes funding payments from users and amm collected fees
...
mod test {
    ...
    fn funding_unsettled_lps_amm_win_test() {
        ...
        assert_eq!(uncapped_funding_pnl, 20321037); //amm revenue
        ...
        let (long_funding, short_funding, amm_pnl) =
            calculate_funding_rate_long_short(&mut market, balanced_funding).unwrap();

        assert_eq!(amm_pnl, 21345628);
        assert_eq!(long_funding, balanced_funding);
        assert_eq!(long_funding, short_funding);
        let new_fees = market.amm.total_fee_minus_distributions;
        assert!(new_fees > QUOTE_PRECISION as i128 / 2);
    ...
    fn funding_unsettled_lps_amm_lose_test() {
        ...
        assert_eq!(uncapped_funding_pnl, -21345628); //amm loses $21
