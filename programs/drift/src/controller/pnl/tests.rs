...
pub fn user_invalid_oracle_position() {
    ...
    let market_map = PerpMarketMap::load_one(&market_account_info, true).unwrap();
    assert!(!market
        .is_price_divergence_ok_for_settle_pnl(oracle_price.price_message.price)
        .unwrap());
    ...
    market
        .amm
        .historical_oracle_data
        .last_oracle_price_twap_5min = oracle_price.price_message.price * 95 / 100;
    create_anchor_account_info!(market, PerpMarket, market_account_info);
    let market_map = PerpMarketMap::load_one(&market_account_info, true).unwrap();
    assert!(!market
        .is_price_divergence_ok_for_settle_pnl(oracle_price.price_message.price)
    ...
    market
        .amm
        .historical_oracle_data
        .last_oracle_price_twap_5min = oracle_price.price_message.price - 789789;
    create_anchor_account_info!(market, PerpMarket, market_account_info);
    let market_map = PerpMarketMap::load_one(&market_account_info, true).unwrap();
...
pub fn is_price_divergence_ok_on_invalid_oracle() {
    ...
    market.amm.mark_std = (oracle_price.price_message.price / 100) as u64;
    market.amm.oracle_std = (oracle_price.price_message.price / 190) as u64;

    assert!(market
        .is_price_divergence_ok_for_settle_pnl(oracle_price.price_message.price)
        .unwrap());

    market.amm.mark_std = (oracle_price.price_message.price / 10) as u64;

    assert!(!market
        .is_price_divergence_ok_for_settle_pnl(oracle_price.price_message.price)
        .unwrap());

    market.amm.oracle_std = (oracle_price.price_message.price * 10) as u64;
