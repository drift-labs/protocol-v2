...
pub mod fulfill_order_with_maker_order {
    ...
    fn taker_oracle_bid_after_auction_crosses_maker_ask() {
        ...
        let taker_price = taker.orders[0]
            .get_limit_price(
                Some(oracle_map.get_price_data(&(oracle_price_key, OracleSource::Pyth)).unwrap().price),
                None,
                slot,
                1,
                false,
    ...
    fn taker_oracle_ask_crosses_maker_bid() {
        ...
        let valid_oracle_price = Some(oracle_map.get_price_data(&(oracle_price_key, OracleSource::Pyth)).unwrap().price);
        let taker_limit_price = taker.orders[0]
            .get_limit_price(
                valid_oracle_price,
                None,
