...
//! | 1 | initialize | [midprice (w), authority (s)] | 36 bytes: market_index (u16), subaccount_index (u16), maker_subaccount ([u8;32]). |
//! | 2 | set_orders | [midprice (w), authority (s)] | valid_until_slot (u64), ask_len (u16), bid_len (u16), then 10×N order entries (tick_count u16 LE, size u64 LE). Tick count != 0; on-tick by construction. |
...
use midprice_book_view::{
    MidpriceBookViewMut, APPLY_FILLS_MARKET_INDEX_SIZE, APPLY_FILLS_NUM_FILLS_SIZE,
    APPLY_FILLS_OPCODE, APPLY_FILLS_SEQ_NUM_SIZE, APPLY_FILL_ENTRY_SIZE, ASK_HEAD_OFFSET,
    ASK_LEN_OFFSET, BID_HEAD_OFFSET, BID_LEN_OFFSET, DISCRIMINATOR_OFFSET, DISCRIMINATOR_SIZE,
    LEVEL_ENTRY_SIZE, LEVEL_ENTRY_SIZE_OFFSET, MAKER_SUBACCOUNT_OFFSET, MARKET_INDEX_OFFSET,
    MAX_ORDERS, PROPAMM_ACCOUNT_DISCRIMINATOR, QUOTE_DATA_LEN_FIELD, QUOTE_DATA_OFFSET_FIELD,
    REFERENCE_PRICE_OFFSET, RESERVED_OFFSET, SEQUENCE_NUMBER_OFFSET, STANDARDIZED_HEADER_SIZE,
    VALID_UNTIL_SLOT_OFFSET, VERSION_OFFSET, VERSION_V1, FLAGS_OFFSET, HEADER_LEN_OFFSET,
};
use pinocchio::{
...
// -- midprice_pino opaque-region constants (offsets 96-151) --
const OPAQUE_AUTHORITY_OFFSET: usize = 96;
const OPAQUE_QUOTE_TTL_OFFSET: usize = 144;
const MIDPRICE_QUOTE_DATA_START: usize = 152;
const MIDPRICE_ACCOUNT_MIN_LEN: usize = 152;

/// Order filled event: emitted for each order filled in apply_fills.
/// Layout: "ordrfill" (8) | market_index (2) | maker_subaccount (32) | abs_index (2) | is_ask (1) | fill_size (8) = 53 bytes.
...
fn emit_order_filled_event(
    ...
    buf[10..42].copy_from_slice(maker_subaccount);
...
fn validate_authority_and_borrow_mut<'a>(
    ...
    if midprice_account.data_len() < MIDPRICE_ACCOUNT_MIN_LEN {
    ...
    if !unsafe {
        authority_matches_32(
            data.as_ptr().add(OPAQUE_AUTHORITY_OFFSET),
...
fn process_initialize_mid_price_account(
    ...
    const INIT_PAYLOAD_LEN: usize = 2 + 2 + 32; // market_index, subaccount_index, maker_subaccount
    ...
    let maker_subaccount: [u8; 32] = payload[4..36].try_into().unwrap();
    ...
    if !unsafe { ptr_is_zero_32(data.as_ptr().add(MAKER_SUBACCOUNT_OFFSET)) } {
    ...
    // Standardized V1 header (96 bytes)
    ...
    data[MAKER_SUBACCOUNT_OFFSET..MAKER_SUBACCOUNT_OFFSET + 32].copy_from_slice(&maker_subaccount);
    data[SEQUENCE_NUMBER_OFFSET..SEQUENCE_NUMBER_OFFSET + 8].fill(0);
    data[VALID_UNTIL_SLOT_OFFSET..VALID_UNTIL_SLOT_OFFSET + 8].fill(0);
    data[REFERENCE_PRICE_OFFSET..REFERENCE_PRICE_OFFSET + 8].fill(0);
    data[QUOTE_DATA_OFFSET_FIELD..QUOTE_DATA_OFFSET_FIELD + 4]
        .copy_from_slice(&(MIDPRICE_QUOTE_DATA_START as u32).to_le_bytes());
    data[QUOTE_DATA_LEN_FIELD..QUOTE_DATA_LEN_FIELD + 4].copy_from_slice(&0u32.to_le_bytes());
    ...
    data[RESERVED_OFFSET..RESERVED_OFFSET + 8].fill(0);

    // Opaque region (offsets 96-151)
    data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32].copy_from_slice(authority_src);
    data[OPAQUE_QUOTE_TTL_OFFSET..OPAQUE_QUOTE_TTL_OFFSET + 8].fill(0);
...
fn process_set_orders(
    ...
    if (payload.len() - 12) % LEVEL_ENTRY_SIZE != 0 {
        log!(64, "midprice set_orders: payload not aligned to entry size");
        return Err(ProgramError::InvalidInstructionData);
    }

    let valid_until_slot = u64::from_le_bytes(payload[0..8].try_into().unwrap());
    ...
    if orders_len != payload_orders_bytes / LEVEL_ENTRY_SIZE {
    ...
    // Validate each order: tick_count != 0.
    // Tick alignment and price overflow are handled by the exchange at match time.
    let orders_start = 12;
    for i in 0..orders_len {
        let base = orders_start + i * LEVEL_ENTRY_SIZE;
        let tick_count = u16::from_le_bytes(payload[base..][..2].try_into().unwrap());
        if tick_count == 0 {
            log!(64, "midprice set_orders: order {} tick_count 0 invalid", i);
            return Err(ProgramError::Custom(AUTH_ERR_ORDER_TICK_OR_SIZE));
        }
    }

    if payload_orders_bytes > data.len() - MIDPRICE_QUOTE_DATA_START {
        log!(64, "midprice set_orders: orders exceed account capacity");
        return Err(ProgramError::InvalidInstructionData);
    }

    data[VALID_UNTIL_SLOT_OFFSET..VALID_UNTIL_SLOT_OFFSET + 8].copy_from_slice(&valid_until_slot.to_le_bytes());
    ...
    data[QUOTE_DATA_LEN_FIELD..QUOTE_DATA_LEN_FIELD + 4]
        .copy_from_slice(&(payload_orders_bytes as u32).to_le_bytes());
    increment_sequence_number(data);
    data[MIDPRICE_QUOTE_DATA_START..MIDPRICE_QUOTE_DATA_START + payload_orders_bytes]
...
fn process_update_mid_price(
    ...
    let reference_price = u64::from_le_bytes(payload[0..8].try_into().unwrap());
    let valid_until_slot = u64::from_le_bytes(payload[8..16].try_into().unwrap());
    let data = validate_authority_and_borrow_mut(program_id, accounts)?;

    data[REFERENCE_PRICE_OFFSET..REFERENCE_PRICE_OFFSET + 8].copy_from_slice(&reference_price.to_le_bytes());
    data[VALID_UNTIL_SLOT_OFFSET..VALID_UNTIL_SLOT_OFFSET + 8].copy_from_slice(&valid_until_slot.to_le_bytes());
    increment_sequence_number(data);
...
fn process_apply_fills(
    ...
    for maker_i in 0..num_makers {
        ...
        let valid_until_slot = u64::from_le_bytes(data[VALID_UNTIL_SLOT_OFFSET..][..8].try_into().unwrap_or([0u8; 8]));
        if valid_until_slot > 0 && current_slot > valid_until_slot {
            log!(
                64,
        ...
        // Copy maker_subaccount for event emission before creating book (which borrows data mutably).
        let maker_subaccount_for_event: [u8; 32] = data[MAKER_SUBACCOUNT_OFFSET..MAKER_SUBACCOUNT_OFFSET + 32]
            .try_into()
            .unwrap_or([0u8; 32]);
        ...
        while start < entries.len() {
            ...
            emit_order_filled_event(expected_market_index, &maker_subaccount_for_event, entry);
...
fn process_set_quote_ttl(
    ...
    data[OPAQUE_QUOTE_TTL_OFFSET..OPAQUE_QUOTE_TTL_OFFSET + 8].copy_from_slice(payload);
...
fn process_close_account(
...

    {
        ...
        let data = unsafe { midprice_account.borrow_unchecked_mut() };
        check_layout_version(data)?;
        if data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32] != *authority.address().as_ref() {
...
fn process_transfer_authority(
    ...
    data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32].copy_from_slice(payload);
...
fn check_layout_version(data: &[u8]) -> ProgramResult {
    ...
    if data[VERSION_OFFSET] != VERSION_V1 {
        return Err(ProgramError::Custom(
            AUTH_ERR_UNSUPPORTED_LAYOUT_VERSION as u32,
        ));
    }
...
mod tests {
    ...
    use core::ptr;
    use midprice_book_view::{
        ASK_HEAD_OFFSET, ASK_LEN_OFFSET, BID_HEAD_OFFSET, BID_LEN_OFFSET, DISCRIMINATOR_OFFSET,
        DISCRIMINATOR_SIZE, LEVEL_ENTRY_SIZE, LEVEL_ENTRY_SIZE_OFFSET, MAKER_SUBACCOUNT_OFFSET,
        PROPAMM_ACCOUNT_DISCRIMINATOR, QUOTE_DATA_LEN_FIELD, QUOTE_DATA_OFFSET_FIELD,
        REFERENCE_PRICE_OFFSET, STANDARDIZED_HEADER_SIZE, VALID_UNTIL_SLOT_OFFSET, VERSION_OFFSET,
        VERSION_V1, FLAGS_OFFSET, HEADER_LEN_OFFSET,
    };
    ...
    /// Build 36-byte init payload: market_index (2), subaccount_index (2), maker_subaccount (32).
    fn init_payload(market_index: u16, subaccount_index: u16) -> Vec<u8> {
        ...
        p.extend_from_slice(&[0xAA; 32]); // maker_subaccount (test placeholder)
        p
    }

    /// Write standardized V1 header + opaque authority into test data buffer.
    fn init_v1_header(data: &mut [u8], authority: &[u8]) {
        data[DISCRIMINATOR_OFFSET..DISCRIMINATOR_OFFSET + DISCRIMINATOR_SIZE]
            .copy_from_slice(&PROPAMM_ACCOUNT_DISCRIMINATOR);
        data[VERSION_OFFSET] = VERSION_V1;
        data[FLAGS_OFFSET] = 0;
        ...
        data[LEVEL_ENTRY_SIZE_OFFSET..LEVEL_ENTRY_SIZE_OFFSET + 2]
            .copy_from_slice(&(LEVEL_ENTRY_SIZE as u16).to_le_bytes());
        data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32].copy_from_slice(authority);
    }
    ...
    /// is stored as tick_count=0 u16 + size u64. Used for apply_fills tests.
    fn make_midprice_data_with_orders(
        authority: &Address,
        market_index: u16,
        _subaccount_index: u16,
        valid_until_slot: u64,
        _quote_ttl_slots: u64,
        ask_sizes: &[u64],
        bid_sizes: &[u64],
    ) -> Vec<u8> {
        ...
        let data_len = MIDPRICE_QUOTE_DATA_START + total_orders * LEVEL_ENTRY_SIZE;
        ...
        data[QUOTE_DATA_OFFSET_FIELD..QUOTE_DATA_OFFSET_FIELD + 4]
            .copy_from_slice(&(MIDPRICE_QUOTE_DATA_START as u32).to_le_bytes());
        let quote_data_len = (total_orders * LEVEL_ENTRY_SIZE) as u32;
        data[QUOTE_DATA_LEN_FIELD..QUOTE_DATA_LEN_FIELD + 4]
            .copy_from_slice(&quote_data_len.to_le_bytes());
        ...
        data[RESERVED_OFFSET..RESERVED_OFFSET + 8].fill(0);
        // Opaque region
        data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32]
            .copy_from_slice(authority.as_ref());
        data[OPAQUE_QUOTE_TTL_OFFSET..OPAQUE_QUOTE_TTL_OFFSET + 8].fill(0);
        // Quote data (tick_count=0 placeholder, only size matters for apply_fills tests)
        let mut off = MIDPRICE_QUOTE_DATA_START;
        for &sz in ask_sizes {
            data[off..off + 2].copy_from_slice(&0u16.to_le_bytes());
            data[off + 2..off + 10].copy_from_slice(&sz.to_le_bytes());
    ...
    fn process_initialize_success() {
        ...
        let mut midprice_backing =
            mock_account_backing(pda, program_id, MIDPRICE_ACCOUNT_MIN_LEN, false, true, 1000, None);
        let mut auth_backing =
            mock_account_backing(authority, test_system_program(), 0, true, false, 0, None);
        let payload = init_payload(market_index, subaccount_index);
        ...
        assert_eq!(data[VERSION_OFFSET], VERSION_V1);
        assert_eq!(
            data[LEVEL_ENTRY_SIZE_OFFSET..LEVEL_ENTRY_SIZE_OFFSET + 2],
            (LEVEL_ENTRY_SIZE as u16).to_le_bytes()
        );
        assert_eq!(
            data[OPAQUE_AUTHORITY_OFFSET..OPAQUE_AUTHORITY_OFFSET + 32],
            authority.as_ref()[..]
    ...
    fn process_initialize_invalid_payload_length() {
        ...
        let ix = [IX_INITIALIZE_MID_PRICE_ACCOUNT, 0u8]; // only 1 byte payload (need 52)
        let result = process_instruction(&program_id, &accounts, &ix);
        assert!(matches!(result, Err(ProgramError::InvalidInstructionData)));
    }
    ...
    fn process_initialize_reinit_fails() {
        ...
        let authority = test_authority();
        let pda = midprice_pda(&program_id, &authority, 0, 0);
        let mut data = vec![0u8; MIDPRICE_ACCOUNT_MIN_LEN];
        init_v1_header(&mut data, authority.as_ref());
        // Set a non-zero maker_subaccount to simulate already initialized.
        data[MAKER_SUBACCOUNT_OFFSET..MAKER_SUBACCOUNT_OFFSET + 32].copy_from_slice(&[0xBB; 32]);
        let mut midprice_backing = mock_account_backing(
            pda,
            program_id,
            MIDPRICE_ACCOUNT_MIN_LEN,
    ...
    fn process_update_mid_price_success() {
        ...
        let reference_price: u64 = 42_000_000;
        let valid_until_slot: u64 = 42;
        let mut ix = vec![IX_UPDATE_MID_PRICE];
        ix.extend_from_slice(&reference_price.to_le_bytes());
        ix.extend_from_slice(&valid_until_slot.to_le_bytes());
        let result = process_instruction(&program_id, &accounts, &ix);
        assert!(result.is_ok());
        let data = unsafe { accounts[0].borrow_unchecked() };
        assert_eq!(
            data[REFERENCE_PRICE_OFFSET..REFERENCE_PRICE_OFFSET + 8],
            reference_price.to_le_bytes()
        );
        assert_eq!(
            data[VALID_UNTIL_SLOT_OFFSET..VALID_UNTIL_SLOT_OFFSET + 8],
            valid_until_slot.to_le_bytes()
        );
    }

    #[test]
    fn update_mid_price_writes_both_fields() {
        let program_id = test_program_id();
        let authority = test_authority();
        let mut data = vec![0u8; MIDPRICE_ACCOUNT_MIN_LEN];
        init_v1_header(&mut data, authority.as_ref());
        ...
        let reference_price: u64 = 100_000;
        let valid_until_slot: u64 = 99;
        let mut ix = vec![IX_UPDATE_MID_PRICE];
        ix.extend_from_slice(&reference_price.to_le_bytes());
        ix.extend_from_slice(&valid_until_slot.to_le_bytes());
    ...
    fn process_set_orders_success_empty_book() {
        ...
        // valid_until_slot=50, ask_len=0, bid_len=0, no orders
        let valid_until_slot: u64 = 50;
        let mut payload = vec![];
        payload.extend_from_slice(&valid_until_slot.to_le_bytes());
    ...
    #[test]
    fn process_set_orders_success_with_orders() {
        let program_id = test_program_id();
        let authority = test_authority();
        let data_len = MIDPRICE_QUOTE_DATA_START + LEVEL_ENTRY_SIZE; // room for 1 order
        let mut data = vec![0u8; data_len];
        init_v1_header(&mut data, authority.as_ref());
        let mut midprice_backing = mock_account_backing(
            test_midprice_account(),
        ...
        // 1 ask: tick_count=1, size 10
        let mut payload = vec![];
        payload.extend_from_slice(&50u64.to_le_bytes()); // valid_until_slot
    ...
    #[test]
    fn process_set_orders_tick_count_zero_fails() {
        let program_id = test_program_id();
        let authority = test_authority();
        let data_len = MIDPRICE_QUOTE_DATA_START + LEVEL_ENTRY_SIZE;
        let mut data = vec![0u8; data_len];
        init_v1_header(&mut data, authority.as_ref());
        let mut midprice_backing = mock_account_backing(
            test_midprice_account(),
    ...
    fn process_set_quote_ttl_success() {
        ...
        let data = unsafe { accounts[0].borrow_unchecked() };
        assert_eq!(
            data[OPAQUE_QUOTE_TTL_OFFSET..OPAQUE_QUOTE_TTL_OFFSET + 8],
            ttl.to_le_bytes()
        );
    }
    ...
    fn process_apply_fills_success_single_fill_reduces_order_size() {
        ...
        let data = make_midprice_data_with_orders(
            ...
            market_index,
            0, // subaccount_index
            0, // valid_until_slot=0 means no deadline
            0,
        ...
        let base = MIDPRICE_QUOTE_DATA_START + 2;
    ...
    fn process_apply_fills_success_bid_fill() {
        ...
        let market_index = 0u16;
        let data =
            make_midprice_data_with_orders(&authority, market_index, 0, 0, 0, &[], &[200u64]);
        let data_len = data.len();
        let mut midprice_backing = mock_account_backing(
            test_midprice_account(),
            program_id,
    ...
    fn process_apply_fills_success_multiple_fills_same_maker() {
        ...
        assert!(result.is_ok());
        let out_data = unsafe { accounts[2].borrow_unchecked() };
        let base0 = MIDPRICE_QUOTE_DATA_START + 2;
        let base1 = MIDPRICE_QUOTE_DATA_START + LEVEL_ENTRY_SIZE + 2;
        assert_eq!(
    ...
    fn process_apply_fills_success_two_makers_second_filled() {
        ...
        let market_index = 1u16;
        let data0 =
            make_midprice_data_with_orders(&authority, market_index, 0, 0, 0, &[100u64], &[]);
        let data1 =
            make_midprice_data_with_orders(&authority, market_index, 0, 0, 0, &[100u64], &[]);
        let len0 = data0.len();
        let len1 = data1.len();
    ...
    fn process_apply_fills_market_index_mismatch_skips_maker() {
        ...
        let authority = test_authority();
        let (mut matcher_backing, mut clock_backing, _) = apply_fills_setup_matcher_clock(50);
        let data = make_midprice_data_with_orders(&authority, 5, 0, 0, 0, &[100u64], &[]);
        let data_len = data.len();
        let mut midprice_backing = mock_account_backing(
            test_midprice_account(),
            program_id,
    ...
    fn process_apply_fills_quote_expired_skips_maker() {
        ...
        let valid_until_slot = 150u64; // current_slot 200 > 150, expired
        let (mut matcher_backing, mut clock_backing, _) =
            apply_fills_setup_matcher_clock(current_slot);
        let data = make_midprice_data_with_orders(
            &authority,
    ...
    fn process_apply_fills_quote_not_expired_applies_fill() {
        let program_id = test_program_id();
        let current_slot = 100u64;
        let valid_until_slot = 110u64; // current_slot 100 <= 110, not expired
        let (mut matcher_backing, mut clock_backing, _) =
            apply_fills_setup_matcher_clock(current_slot);
        let authority = test_authority();
        let data = make_midprice_data_with_orders(
    ...
    fn process_apply_fills_payload_trailing_bytes_fails() {
        ...
        let (mut matcher_backing, mut clock_backing, _) = apply_fills_setup_matcher_clock(50);
        let authority = test_authority();
        let data = make_midprice_data_with_orders(&authority, 1, 0, 0, 0, &[100u64], &[]);
        let data_len = data.len();
        let mut midprice_backing = mock_account_backing(
            test_midprice_account(),
            program_id,
    ...
    fn apply_fill_entry_ask_flag_with_bid_index_rejected() {
        ...
        let data = make_midprice_data_with_orders(
            &authority, 1, 0, 0, 0, &[100u64], &[200u64],
        );
        let data_len = data.len();
        let mut midprice_backing = mock_account_backing(
