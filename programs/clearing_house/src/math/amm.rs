...
pub fn update_oracle_mark_spread_twap(amm: &mut AMM, now: i64, new_data: i128) -> ClearingHouseResult<i128> {
    ...
    let new_twap = calculate_twap(new_data,
...
pub fn calculate_twap(new_data: i128, old_data: i128, new_weight: i128, old_weight: i128) -> ClearingHouseResult<i128> {
    let denominator = new_weight.checked_add(old_weight).ok_or_else(math_error!())?;
    let prev_twap_99 = old_data
        .checked_mul(old_weight)
        .ok_or_else(math_error!())?;
    let latest_price_01 = new_data
