...
fn no_fulfill_with_amm_jit_taker_long() {
    ...
    assert_eq!(taker_stats.taker_volume_30d, 101263863);
...
fn fulfill_with_amm_jit_taker_short() {
    ...
    assert_eq!(market_after.amm.total_exchange_fee, 40748);
...
fn fulfill_with_amm_jit_taker_long() {
    ...
    assert_eq!(reserve_price_after, 101_517_045);
    assert_eq!(base_asset_amount, BASE_PRECISION_U64);
...
fn fulfill_with_amm_jit_full_long() {
    ...
    loop {
        ...
        let is_complete =
            crate::math::auction::is_auction_complete(taker.orders[0].slot, auction_duration, slot)
                .unwrap();
        if is_complete {
            break;
        }
